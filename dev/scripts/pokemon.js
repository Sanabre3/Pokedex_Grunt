...
class PokemonManager {
  ...
  createPokemonCard(pokemon) {
    ...
    const types = (pokemon.types || [])
      .map(
        (type) =>
          `<span class="type type-${type.type.name}">${this.capitalizeFirst(
            type.type.name
          )}</span>`
      )
      .join("");
    ...
    card.innerHTML = `
      ...
      <div class="stat"><span class="stat-name">HP</span><span class="stat-value">${
        pokemon.stats?.[0]?.base_stat ?? "-"
      }</span></div>
      <div class="stat"><span class="stat-name">ATK</span><span class="stat-value">${
        pokemon.stats?.[1]?.base_stat ?? "-"
      }</span></div>
      <div class="stat"><span class="stat-name">DEF</span><span class="stat-value">${
    ...
    <div class="pokemon-detail">
        <div class="detail-header">
          ...
          ${(pokemon.stats || [])
            ...
            <div class="stat-bar">
            <div class="stat-info">
            <span class="stat-name">${this.formatStatName(
              stat.stat.name
            )}</span>
            ...
            <div class="measurement"><span class="label">Altura:</span><span class="value">${
              pokemon.height / 10
            }m</span></div>
            <div class="measurement"><span class="label">Peso:</span><span class="value">${
              pokemon.weight / 10
            }kg</span></div>
            </div>
            </div>
            ...
            ${(pokemon.moves || [])
              ...
              .map(
                (move) =>
                  `<span class="move-tag">${this.capitalizeFirst(
                    move.move.name
                  )}</span>`
              )
              .join("")}
